...
pub static ROOK_MOVES: Lazy<Box<[[Bitboard; 4096]]>> =
...
fn precompute_rook_magic_bitboards() -> Box<[[Bitboard; 4096]]> {
    ...
    let mut rook_moves = vec![[Bitboard(0); 4096]; 64].into_boxed_slice();
    // same for every square
    let max_blocker_count = ROOK_ATTACK_MASKS[0].count_ones() as u64;
    for square in 0..64 {
        for blockers in 0..1 << max_blocker_count {
            let mut attack_bitboard = ROOK_ATTACK_MASKS[square];
            let mut blocker_bitboard = Bitboard(0);

            let square_rook_moves = get_rook_moves_traditional(square, blocker_bitboard);

            while attack_bitboard.0 != 0 {
                let blocker_index = attack_bitboard.pop_lsb();
            ...
            rook_moves[square][blocker_bitboard.0 as usize] = square_rook_moves;
        }
    }
    rook_moves
}
...
mod tests {
    ...
    fn correct_rook_bitboards() {
        let square = 0;
        let mut blocker_bitboard = Bitboard(0b00001);
        blocker_bitboard &= ROOK_ATTACK_MASKS[square];
        blocker_bitboard *= ROOK_MAGIC_NUMBERS[square];
        blocker_bitboard >>= 64 - ROOK_SHIFT_AMOUNT[square];
        println!("{}", get_rook_moves_traditional(square, blocker_bitboard));

        assert_eq!(
            ROOK_MOVES[square][blocker_bitboard.0 as usize],
            get_rook_moves_traditional(square, blocker_bitboard)
        );
    }
}
